		...
		// RODO: Variable to store if we have an active fase
		var faceActive = false;
		// Variable to set and clear the face active timeout
		var faceActiveTimeout;
		var faceActiveTimeoutDuration = 2000;
			...
			var processfn = function(video, dt) {
				...
				for(i=0; i<dets.length; ++i)
					...
					if(dets[i][3]>50.0)
					{
						...
						// set the faceActive variable to true
						// set a time out to de-activate the face after a duration
						if (!faceActive){
							displayText(); 
							faceActive = true;
							faceActiveTimeout = setTimeout(() => {
								faceActive = false;
							}, faceActiveTimeoutDuration);
						} else {
							// reset the timeout
							clearTimeout(faceActiveTimeout);
							faceActiveTimeout = setTimeout(() => {
								faceActive = false;
							}, faceActiveTimeoutDuration);
						}
		...
		{ 
			...
			var text = document.getElementById("textBox");
				...
				setTimeout(function(){ text.value="because of this, it has the strongest ties to memory." }, 12000);
